...
void BlockFall::print_grid() {
    ...
    std::cout << "High Score: " << (leaderboard.head_leaderboard_entry != nullptr ? leaderboard.head_leaderboard_entry->score : 0)<< "\n";
    grid_printer();
    std::cout << "\n";
}

void BlockFall::grid_printer() {
    for (auto &row : grid) {
        for (int i : row) {
...
void BlockFall::prompt_win() {
    std::cout << "YOU WIN!\nNo more blocks.\nFinal grid and score:\n\n";
    print_grid();
    leaderboard.print_leaderboard();
}

void BlockFall::prompt_out_of_com() {
    std::cout << "GAME FINISHED!\nNo more commands.\nFinal grid and score:\n\n";
    print_grid();
    leaderboard.print_leaderboard();
}

void BlockFall::prompt_loss() {
...
void BlockFall::print_block() {
    for (int i = 0; i < active_rotation->shape.size(); ++i) {
        for (int j = 0; j < active_rotation->shape[0].size(); ++j) {
            std::cout << (active_rotation->shape[i][j] ? occupiedCellChar : unoccupiedCellChar);
        }
        std::cout << "\n";
    }
    std::cout << "\n";
